...
| [Cryolite の雀魂シミュレーション検証記録](https://gist.github.com/Cryolite/a026f41713f6a7ca88713737f5c2cfb6) | 牌譜 ID 付きの検証記録 | corner case候補とsource牌譜IDを得る。[ADR-0011](../adr/0011-one-pass-majsoul-conformance.md)に従って手書き最小CI testとCI外full-record testへ利用する |
...
雀魂公式詳細ルール
  ...
  -> corner caseを手書きの最小fixtureとしてGitへ保存してCI test
  -> conformance appがlocal `majsoul-record`を逐次decode・project
  -> 中間`game_log`を必須にせずLizhiSimとcheckpoint比較
  -> 完全な`majsoul-record` corpusをGit外に保持し定期実行
  -> red -> green -> refactor
  -> LizhiSim の event と元牌譜を照合
```

Kanachanの挙動だけを期待値にしない。最小fixtureはsource牌譜IDと構成意図を記録し、full-record testは`majsoul-record`から毎回decodeしてsource event単位でLizhiSimと比較する。
