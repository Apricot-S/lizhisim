...
- [1. 識別子の原則](#1-識別子の原則)
- [2. 確定済みの麻雀用語](#2-確定済みの麻雀用語)
- [3. 確定済みの英語用語](#3-確定済みの英語用語)
- [4. 使用禁止・廃止表記](#4-使用禁止廃止表記)
- [5. ユーザー決定待ち](#5-ユーザー決定待ち)
- [6. 曖昧語を避ける](#6-曖昧語を避ける)
...
| 対象 | Rust variant | Compact notation |
|---|---|---|
| 萬子1〜9 | `M1`〜`M9` | `1m`〜`9m` |
| 赤五萬 | `M0` | `0m` |
| 筒子1〜9 | `P1`〜`P9` | `1p`〜`9p` |
...
| 索子1〜9 | `S1`〜`S9` | `1s`〜`9s` |
| 赤五索 | `S0` | `0s` |
| 東 | `Z1`（comment: `dong`） | `Z1` |
| 南 | `Z2`（comment: `nan`） | `Z2` |
| 西 | `Z3`（comment: `xi`） | `Z3` |
| 北 | `Z4`（comment: `bei`） | `Z4` |
| 白 | `Z5`（comment: `bai`） | `Z5` |
| 發 | `Z6`（comment: `fa`） | `Z6` |
| 中 | `Z7`（comment: `zhong`） | `Z7` |
